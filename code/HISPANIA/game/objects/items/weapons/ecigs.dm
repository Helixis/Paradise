	...
	if(ishuman(loc))
		...
		if (src == C.wear_mask && C.check_has_mouth()) // if it's in the human/monkey mouth, transfer reagents to the mob
			...
			for (var/datum/reagent/R in ec_cartridge.reagents.reagent_list)
				ec_cartridge.reagents.trans_id_to(C, R.id, max(REAGENTS_METABOLISM / reagents.reagent_list.len, 0.1)) //transfer at least .1 of each chem
...
/obj/item/reagent_containers/ecig_cartridge/examine(mob/user as mob)//to see how much left
	. = ..()
	. += "<span class='notice'>Has [reagents.total_volume] units of liquid remaining.</span>"
