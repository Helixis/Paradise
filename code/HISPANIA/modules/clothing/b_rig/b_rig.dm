...
/obj/item/b_rig/proc/suit_is_deployed()
	if(!istype(wearer) || src.loc != wearer || wearer.belt != src)
		return 0
...
/obj/item/b_rig/proc/seal(mob/living/user)
	if(sealing)
		return 0
	if(!wearer || !user)
		return
...
/obj/item/b_rig/proc/unseal(mob/living/user)
	if(sealing)
		return 0
	if(!wearer || !user)
		return
...
/obj/item/b_rig/proc/update_component_sealed()
...
/obj/item/b_rig/process()
	...
	if(malfunction_delay > 0)
		...
		malfunctioning--
		malfunction()
	if(!istype(wearer) || loc != wearer || wearer.belt != src || !(flags & NODROP) || !cell || cell.charge <= 0)
...
/obj/item/b_rig/proc/check_power_cost(var/mob/living/user, var/cost, var/use_unconcious, var/user_is_ai)
	if(!istype(user))
		return 0
	var/fail_msg
	if(!user_is_ai)
...
/obj/item/b_rig/proc/check_suit_access(var/mob/living/carbon/human/user)
...
/obj/item/b_rig/Topic(href,href_list)
...
/obj/item/b_rig/proc/deploy(mob/user)
	if(!wearer || !user)
		return 0
...
/obj/item/b_rig/dropped(var/mob/user)
	..()
...
/obj/item/b_rig/proc/malfunction()
	return 0
...
/obj/item/rig/get_brig()
	return src

/mob/living/carbon/human/get_brig()
	if(istype(belt,/obj/item/b_rig))
		return belt
	else
		return null

/obj/item/b_rig/proc/can_touch(var/mob/user, var/mob/wearer)
	if(!user)
		return 0
